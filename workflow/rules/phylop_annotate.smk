...
rule vcf2bedspm:
    ...
    shell:
        """
        zcat {input} | awk -FS='\t' -v OFS='\t' -v sample='{wildcards.sample}' '{{print $0,sample":"$1":"$2":"$4":"$5}}' |
...
rule vcf2bedsim:
    ...
    shell:
        ...
        zgrep -v '^#' {input} |
        awk -FS='\t' -v OFS='\t' -v sample='{wildcards.sample}' '{{ if (length($4)==length($5)) {{ print $0,sample":"$1":"$2":"$4":"$5}} }}' |
